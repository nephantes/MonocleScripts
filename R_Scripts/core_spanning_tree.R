#First run plot_spanning_tree(HSMM) -> plot
FullCoreGroup <- unlist(CoreGroup_3match_24size_n7) #The full list of cells from core groupings!!
FullCoreGroup <- FullCoreGroup[1:length(plot$data$target)]
IndxFCG <- match(FullCoreGroup, plot$data$target)

CoreGroup1 <- CoreGroup_3match_24size_n7[[1]]
#CoreGroup1 <- CoreGroup1[1:length(plot$data$sample_name)]
CoreGroup1Names <-plot$data$sample_name
CoreGroup1Names[!(plot$data$sample_name %in% CoreGroup1)] = "Ungrouped"
...
core_spanning_tree <- function (cds, coreGroup =NULL, x = 1, y = 2, color_by = "State", show_tree = TRUE, 
